      ...
      <body>
        <div id="content">
          <div id="content-head">
            <h1>XML Sitemap</h1>
            <p>
              Gerado por <a href="https://aioseo.com/?utm_source=WordPress&#038;utm_campaign=liteplugin&#038;utm_medium=xml-sitemap" target="_blank">All in One SEO</a>, este é um sitemap XML, destinado a ser consumido por mecanismos de pesquisa como Google ou Bing.<br/>
            ...
            <p>
              <xsl:choose>
                <xsl:when test="$fileType='Sitemap'">
                  Este sitemap contém <xsl:value-of select="count(sitemap:urlset/sitemap:url)"></xsl:value-of> URLs.</xsl:when>
                <xsl:otherwise>Este índice de sitemap contém <xsl:value-of select="count(sitemap:sitemapindex/sitemap:sitemap)"></xsl:value-of> sitemaps.</xsl:otherwise>
              </xsl:choose>
            </p>
          </div>
          <xsl:choose>
